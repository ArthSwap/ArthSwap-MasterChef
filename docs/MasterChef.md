...
- `arswToken`:
  The ARSW token contract address.
...
- `overwrite`:
  True if rewarder should be `set`. Otherwise `rewarder` is ignored.
...
- `lpSupply`:
  sum deposit amount of the LP token by the pool
...
- `to`:
  The receiver of `amount` deposit benefit.
...
- `to`:
  Receiver of the LP tokens and ARSW rewards.
